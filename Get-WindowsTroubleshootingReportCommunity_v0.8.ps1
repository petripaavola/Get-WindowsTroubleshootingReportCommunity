...
.EXAMPLE
    Get-WindowsTroubleshootingReportCommunity.ps1 -LastDays 7
    This command retrieves all known events from the last 7 days from live Windows logs.

.EXAMPLE
    Get-WindowsTroubleshootingReportCommunity.ps1 -LastMinutes 5 -AllEvents
    This command retrieves all events from the last 5 minutes. This gathers all events - known and unknown.
...
function Read-EventLog {
	...
	if($EventLog) {
		...
		if($AllEvents) {
			...
			Write-Host "Found $($KnownEventRulesIds.Count) known Event Id's"
...
Write-Host "Selected KnownRules Categories to include:`n$($EventRulesArray.CategoryName -join "`n")"
...
# For now we only convert Intune App names
if($True) {
	$IntunePolicies = $null
	$FoundGUIDs = @{}
	...
	Foreach($entry in $observedTimeline) {
		...
		if($entry.LogName -like '*.log') {
			...
			if($entry.Message -match 'Get policies = (.*)') {
				...
				# Add Intune Policies to $FoundGUIDs hashtable. Use ID as key and save whole object to value
				Foreach($Policy in $IntunePolicies) {
					$FoundGUIDs[$Policy.id] = $Policy
				}
			...
			if($FoundGUIdS) {
				...
				$GUIDs = $entry.Message | Select-String -Pattern '[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}' -AllMatches | ForEach-Object { $_.Matches.Value }

				# Replace GUIDs with human readable names
				Foreach($GUID in $GUIDs) {
					if($FoundGUIDs.ContainsKey($GUID)) {
						if($FoundGUIDs[$GUID].Name) {
							$entry.Message = $entry.Message -replace $GUID, "$($FoundGUIDs[$GUID].Name)"
						}

						if($FoundGUIDs[$GUID].displayName) {
							$entry.Message = $entry.Message -replace $GUID, $FoundGUIDs[$GUID].displayName
						}
					}
				}
			}
