...
function Read-LogFile {
	...
	Foreach($Line in $LogFileLines) {
		...
		if($Line -Match $IntuneCustomInventoryAdaptorLogEntryRegex) {
			...
			Write-Host "Log file recognized as Intune custom InventoryAdaptor -type"
		...
		if($Line -Match $IntuneCustomInventoryHarvesterLogEntryForNonUSARegex) {
			...
			Write-Host "Log file recognized as Intune custom InventoryHarvester non-USA dateTime -type"
		...
		if($Line -Match $IntuneCustomInventoryHarvesterLogEntryForUSARegex) {
			...
			Write-Host "Log file recognized as Intune custom InventoryHarvester USA dateTime -type"
...
if(Test-Path "$PSScriptRoot\knownGUIDs") {
	...
	if($knownGUIDsFiles) {
		Write-Host "Import KnownGUIDs files"
		...
		Foreach($File in $knownGUIDsFiles) {
			...
			if($Success) {
				Write-Host "Imported $($KnownGUIDsArray.Count) KnownGUIDs" -ForegroundColor 'Green'
				...
				Write-Host "Failed to import KnownGUIDs.json" -ForegroundColor 'Yellow'
			}
		}
	} else {
